...
ConsoleGamePong::ConsoleGamePong() : ball_ptr_(std::make_shared<Ball>()), player1_ptr_(std::make_shared<Paddle>()), player2_ptr_(std::make_shared<Paddle>()) {}
...
bool ConsoleGamePong::Draw() {
	...
	std::cout << "Score 1: " << score1_ << std::endl << "Score 2: " << score2_ << std::endl;
	return true;
}
...
bool ConsoleGamePong::Logic() {
	...
	if (ball_ptr_->GetY() == height_ - 1) {
		ball_ptr_->ChangeDirection(ball_ptr_->GetDirection() == DOWNRIGHT ? UPRIGHT : UPLEFT);
	}
	//top wall
	if (ball_ptr_->GetY() == 0) {
		ball_ptr_->ChangeDirection(ball_ptr_->GetDirection() == UPRIGHT ? DOWNRIGHT : DOWNLEFT);
	}
	//right wall
